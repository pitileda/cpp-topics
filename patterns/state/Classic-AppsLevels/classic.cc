...
class ApplicationManager {
 ...
 public:
  ...
  std::optional<AppId> activeApp_;

  ApplicationManager(StateMachine& sm) : sm_(sm), activeApp_(std::nullopt) {
    // activate
    sm_.addTransition(
  ...
  void printApps() {
    for (auto it = applications_.begin(); it != applications_.end(); ++it) {
      auto app = it->second;
      std::cout << app->type_ << " ," << app->id_ << ", " << app->getState() << std::endl;
    }
  }
};
...
void StateMachine::handleEvent(const Event& event, App app,
                               ApplicationManager& am) {
  auto deactivateCurrent = [&]() {
    if (am.activeApp_ != std::nullopt) {
      auto curr_app = am.applications_[am.activeApp_.value()];
      if (curr_app) {
  ...
  for (const auto& transition : transitions_) {
    if (transition.trigger_ == event.type_) {
      ...
      if (event.type_ == EventType::Deactivate) {
        ...
        if (app->id_ != am.activeApp_.value()) {
          return;
        }
        deactivateCurrent();
      }
