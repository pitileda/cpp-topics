...
void do_smth(bool b, int i, std::string sv) {
  std::cout << "do somethig with " << b << ", " << i << ", " << sv << std::endl;
}
...
void wrapper(Ts&&... args) {
  ...
  std::tuple<bool, int, std::string> f_args;
  ...
  auto bind_arg = [&](auto&& arg) {
    ...
    // else {
    // 	static_assert(std::is_null_pointer<decltype(arg)>::value, "Invalid
    // argument type");
    // }
  };
...
int main() {
  using std::cout;
  using std::endl;
  wrapper(true, 12, std::string{"Hello"});
  wrapper(12, true, std::string{"Hello"});
  wrapper(std::string{"Hello"}, true, 12);
