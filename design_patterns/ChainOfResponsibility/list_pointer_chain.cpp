...
public:
  ...
  explicit Modificator(Creature &Next) : creature(Next) {
    next_modificator = modificators.end();
  }

  void add(std::unique_ptr<Modificator> Next) {
    modificators.push_back(std::move(Next));
  }

  void push_front(std::unique_ptr<Modificator> Next) {
    modificators.push_front(std::move(Next));
  }

  virtual bool handle() {
    for (auto& mod: modificators) {
      if (!mod->handle()) {
        return false;
      }
    }
...
private:
  ...
  std::list<std::unique_ptr<Modificator>>::iterator next_modificator;
};
...
int main() {
  ...
  root.add(std::make_unique<Duplicator>(creature));
  root.push_front(std::make_unique<Adder>(creature));
  root.handle();
